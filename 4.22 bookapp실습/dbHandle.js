...
// 데이터베이스 생성 및 오픈
function openDB(){
   db = window.openDatabase('bookDB', '1.0', '북DB', 1024*1024); //widow.openDatabase("DB이름","버전","DB표시이름","DB크리[,성공콜백함수]")
   console.log('1_DB 생성...'); 
} 
// 테이블 생성 트랜잭션 실행
function createTable() {
 ...
 function insertBook(){ 
    db.transaction(function(tr){
     	...
     	tr.executeSql(insertSQL, [type, name], function(tr, rs){    //resultset ==> rs sql문장을 수행한 결과로 받게 되는 DB반환 객체이다.  
      	    console.log('3_ 책 등록...no: ' + rs.insertId);         // rs.insertid => insert문 실행 결과로 자동 생성된 id 
  ...
  db.transaction(function(tr){
 	var selectSQL = 'select * from book';    
  	tr.executeSql(selectSQL, [], function(tr, rs){    
       console.log(' 책 조회... ' + rs.rows.length + '건.'); //rs.row -> select문 실행 결과로 생성된 결과 집합 안의 투플(행) 리스트(객체)
       if (position == 'first') {
       	  if(index == 0) 
       	  ...
       	  if(index == rs.rows.length-1) //row.length => 결과 집합 안의 투플(행)들의 전체 수 
       	  	alert('더 이상의 도서가 없습니다');          	
		      else
		      	index = ++index;
       }
       ...
       $('#bookType4').val(rs.rows.item(index).type); row.item(첨자).컬럼명 : 결과 집합 안의 첨자에 해당하는 투플(행)의 특정 컬럼값 
